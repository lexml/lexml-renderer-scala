...
trait CharStreamSource extends StreamSource {
  def encoding = "UTF-8"
  def write(w: Writer)
  final override def write(os: OutputStream) {
...
object TextSectionSource {
  ...
  def reducePath(p : String) : String = reducePath(new File(p).getParentFile)
