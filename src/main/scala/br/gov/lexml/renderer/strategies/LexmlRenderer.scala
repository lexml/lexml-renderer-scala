...
import br.gov.lexml.renderer.terms.XML.Conversions.*
...
object XhtmlRenderer:
  ...
  val hasXmlBase: Strategy = rule[XObject] {
    case e: XElem if e.attributes.contains("xml:base") => e : XObject
  }
  ...
  val dealWithRotuloAndOmissis: Strategy =
    def f(l: List[XObject]): Option[List[XObject]] =
      val (rl1, r1) = l.foldRight(List[XObject](), false) {
        case (XElem("Omissis", _, attrs, _), (r, _)) =>
          (XElem("p", "", attrs.view.filterKeys(_ == "id").toMap + ("class" -> "omissis"),
            List(XText("....."))) :: r, true)
      ...
      if r1 then Some(rl1) else None
    reduce {
      rule ({
        case e: XElem if f(e.contents).isDefined =>
          f(e.contents).collect[XElem](ll => e.copy(contents = ll)).get
      } : XElem ==> XElem)
    }
  ...
  def makeRenderer(opts: RenderOptions = baseRenderOptions): Renderer =
    ...
    (e: Elem) => {
      ...
      val fl =
        ...
        yield
          val body : XElem = XElem("body", "", Map(), els)
          val ids = getIds(body)
          (n, ids, XElem("body", "", Map(), els) : XElem)
      val idMap = (
          for
            (n, ids, _) <- fl
            id <- ids
          yield (id, TextSectionSource.fname(n))
        ).toMap
      val cssFiles = List(
